'use strict';

var Sidebar = require('vitepress-plugin-sidebar-resolve');
var node_path = require('node:path');
var node_fs = require('node:fs');
var index = require('../../helper/dist/index.js');
var helper = require('./helper.js');
var matter = require('gray-matter');

const transformData = (data) => {
  const siteConfig = globalThis.VITEPRESS_CONFIG;
  const { themeConfig } = siteConfig.userConfig;
  const { frontmatter, url, excerpt } = data;
  if (frontmatter.date) frontmatter.date = index.formatDate(frontmatter.date);
  return {
    url,
    frontmatter,
    author: themeConfig.author,
    title: getTitle(data),
    date: getDate(data, siteConfig.srcDir),
    excerpt,
    capture: getCaptureText(data)
  };
};
const transformRaw = (posts) => {
  const siteConfig = globalThis.VITEPRESS_CONFIG;
  const { locales = {} } = siteConfig.userConfig;
  const postsData = resolvePosts(posts);
  const localesKeys = Object.keys(locales);
  if (!localesKeys.length) return postsData;
  const postsLocale = {};
  localesKeys.filter((localesKey) => localesKey !== "root").forEach((localesKey) => {
    const localePosts = posts.filter((post) => post.url.startsWith(`/${localesKey}`));
    postsLocale[localesKey] = resolvePosts(localePosts);
  });
  const rootPosts = posts.filter((post) => !localesKeys.some((localesKey) => post.url.startsWith(`/${localesKey}`)));
  postsLocale["root"] = resolvePosts(rootPosts);
  return { ...postsData, locales: postsLocale };
};
const resolvePosts = (posts) => {
  const originPosts = helper.filterPosts(posts);
  const sortPostsByDateAndSticky = helper.getSortPostsByDateAndSticky(originPosts);
  const sortPostsByDate = helper.getSortPostsByDate(originPosts);
  const groupPostsByYear = helper.groupByYear(sortPostsByDate);
  const groupPostsByYearMonth = helper.groupByYearMonth(sortPostsByDate);
  const groupPosts = helper.getGroupPosts(sortPostsByDateAndSticky);
  const groupCards = helper.getGroupCards(groupPosts);
  return {
    allPosts: posts,
    originPosts,
    sortPostsByDateAndSticky,
    sortPostsByDate,
    groupPostsByYear,
    groupPostsByYearMonth,
    groupPosts,
    groupCards
  };
};
function getTitle(post) {
  if (post.frontmatter.title) return post.frontmatter.title;
  const { content = "" } = matter(post.src || "", {});
  const splitName = node_path.basename(post.url).split(".");
  const name = splitName.length > 1 ? splitName[1] : splitName[0];
  return Sidebar.getTitleFromMd(content) || name || "";
}
function getDate(post, srcDir) {
  const { frontmatter, url } = post;
  if (frontmatter.date) return frontmatter.date;
  const filePath = node_path.join(srcDir, `${url.endsWith("/") ? `${url}/index` : url.replace(/\.html$/, "")}.md`);
  const stat = node_fs.statSync(filePath);
  return index.formatDate(stat.birthtime || stat.atime);
}
const getCaptureText = (post, count = 300) => {
  const { content = "" } = matter(post.src || "", {});
  return content?.replace(/^#+\s+.*/, "")?.replace(/#/g, "")?.replace(/!\[.*?\]\(.*?\)/g, "")?.replace(/\[(.*?)\]\(.*?\)/g, "$1")?.replace(/\*\*(.*?)\*\*/g, "$1")?.replace(/\[\[TOC\]\]/g, "")?.replace(/:::.*?(\n|$)/g, "")?.replace(/<!-- more -->/g, "")?.split("\n")?.filter((v) => !!v)?.join("\n")?.replace(/>(.*)/, "")?.replace(/</g, "&lt;").replace(/>/g, "&gt;")?.trim()?.slice(0, count);
};

exports.getCaptureText = getCaptureText;
exports.getDate = getDate;
exports.getTitle = getTitle;
exports.transformData = transformData;
exports.transformRaw = transformRaw;
