...
const renderShareCard = (shareCard, info, base) => {
  ...
  return `
    ...
    (card) => `
            <${card.link ? "a" : "span"}
              href="${index.withBase(base, card.link)}" target=${target}
             ...
             >
              ${card.avatar ? `<img src="${index.withBase(base, card.avatar)}" alt="${card.name}" class="no-preview">` : ""}
              <div>
...
exports.default = shareCardPlugin;
